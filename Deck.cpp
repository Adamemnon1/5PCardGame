	...
	if (ifs.is_open()){
		...
		while (getline(ifs, nextLine)){
			...
			while (iss >> c){
				...
				if (c.length() == 2 || c.length() == 3){  //ks01	//JZ: magic numbers
...
ostream & operator<<(ostream & out, const Deck & d) {
	for (unsigned int i = 0; i < d.deck.size(); ++i){
		//JZ: use your Card toString or operator<< instead of all of this...
		Card card = d.deck[i];
		string suit;
		string value;
		switch (card.s){
...
Deck::Deck() {	//use base/member initialization list here.
	deck = {};
}
